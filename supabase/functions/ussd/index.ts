...
async function setSession(sessionId: string, phone: string, step: string, data: Record<string, unknown> = {}) {
  const { error } = await supabase
    .from('ussd_sessions')
    .upsert({ session_id: sessionId, phone, step, data, updated_at: new Date().toISOString() }, { onConflict: 'session_id' });
...
function CON(message: string) {
  return new Response(message, { headers: { 'Content-Type': 'text/plain', 'Freeflow': 'FC' } });
}

function END(message: string) {
  return new Response(message, { headers: { 'Content-Type': 'text/plain', 'Freeflow': 'FB' } });
}
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!sessionId || !phoneNumber) return errorResponse('Missing sessionId or phoneNumber');
    ...
    if (step === 'main_menu') {
      switch (input) {
        case '1': return await showRoutes(sessionId, phoneNumber);
        case '2': return await showMyBookings(sessionId, phoneNumber);
        case '3': return await startChangeTicket(sessionId, phoneNumber);
        case '4': return await startCancelTicket(sessionId, phoneNumber);
        case '5':
          await deleteSession(sessionId);
          return END('BusLink Malawi Help\nBook bus tickets via USSD.\nPay with Airtel Money.\nFor support: 0888-BUS-HELP');
        default:
    ...
    if (step === 'select_route') {
      ...
      if (isNaN(idx) || idx < 0 || idx >= routes.length) return CON('Invalid selection. Please pick a number from the list.');

      const selectedRoute = routes[idx];
      const { data: route } = await supabase.from('routes').select('one_way_price, return_price').eq('id', selectedRoute.id).single();
    ...
    if (step === 'select_ticket_type') {
      if (input !== '1' && input !== '2') return CON('Invalid choice.\n1. One Way\n2. Return');
    ...
    if (step === 'select_date') {
      ...
      const idx = parseInt(input) - 1;
      if (isNaN(idx) || idx < 0 || idx >= trips.length) return CON('Invalid selection. Please pick a number.');

      const selectedTrip = trips[idx];
      const { data: route } = await supabase.from('routes').select('one_way_price, return_price').eq('id', sessionData.route_id as string).single();
    ...
    if (step === 'confirm_booking') {
      ...
      if (input !== '1') return CON('1. Pay with Airtel Money\n2. Cancel');
      ...
      await supabase.from('seat_locks').insert({ trip_id: sessionData.trip_id as string, booking_id: booking.id, phone: phoneNumber });

      const txRef = `USSD-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
      await supabase.from('payments').insert({ booking_id: booking.id, amount, transaction_reference: txRef, status: 'pending' });
    ...
    if (step === 'change_ticket_input') {
      ...
      if (!booking) { await deleteSession(sessionId); return END('Ticket not found or not eligible for change.'); }
      ...
      if (!altTrips || altTrips.length === 0) { await deleteSession(sessionId); return END('No alternative trips available.'); }

      let menu = 'Select new travel date:\n';
      altTrips.forEach((t, i) => { menu += `${i + 1}. ${t.travel_date} ${t.departure_time || ''} (${t.available_seats} seats)\n`; });

      await setSession(sessionId, phoneNumber, 'change_ticket_select', {
        booking_id: booking.id, ticket_code: ticketCode,
    ...
    if (step === 'change_ticket_select') {
      ...
      const idx = parseInt(input) - 1;
      if (isNaN(idx) || idx < 0 || idx >= altTrips.length) return CON('Invalid selection.');

      const newTrip = altTrips[idx];
      await supabase.from('bookings').update({ trip_id: newTrip.id }).eq('id', sessionData.booking_id as string);
      ...
      await supabase.from('sms_logs').insert({
        ...
        sms_type: 'booking_changed', status: 'queued', booking_id: sessionData.booking_id as string,
      });
    ...
    if (step === 'cancel_ticket_input') {
      ...
      if (!booking) { await deleteSession(sessionId); return END('Ticket not found or not eligible for cancellation.'); }

      await setSession(sessionId, phoneNumber, 'cancel_ticket_confirm', {
        booking_id: booking.id, ticket_code: ticketCode, trip_id: booking.trip_id,
      });
    ...
    if (step === 'cancel_ticket_confirm') {
      if (input === '2') { await deleteSession(sessionId); return END('Cancellation aborted.'); }
      ...
      if (trip) {
        await supabase.from('trips').update({ available_seats: trip.available_seats + 1, status: 'active' }).eq('id', sessionData.trip_id as string);
      }
      ...
      await supabase.from('sms_logs').insert({
        ...
        sms_type: 'booking_cancelled', status: 'queued', booking_id: sessionData.booking_id as string,
      });
...
async function showRoutes(sessionId: string, phone: string) {
  const { data: routes } = await supabase.from('routes').select('id, origin, destination').eq('status', 'active').limit(8);

  if (!routes || routes.length === 0) { await deleteSession(sessionId); return END('No routes available at this time.'); }
...
async function showMyBookings(sessionId: string, phone: string) {
  ...
  if (!bookings || bookings.length === 0) return END('No active bookings found.');
