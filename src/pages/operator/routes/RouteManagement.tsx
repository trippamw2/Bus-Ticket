...
import { Plus, Pencil, Loader2, Ban, CheckCircle, Calendar, History } from "lucide-react";
...
const RouteManagement = () => {
  ...
  useEffect(() => { if (operator) { fetchRoutes(); fetchSeasonalPricing(); } }, [operator, operator?.id]);
  useEffect(() => { if (selectedRouteForHistory) fetchPriceHistory(selectedRouteForHistory); }, [selectedRouteForHistory]);

  const fetchRoutes = async () => {
    if (!operator) return;
    const { data } = await supabase.from("routes").select("*").eq("operator_id", operator.id).order("created_at", { ascending: false });
  ...
  const fetchSeasonalPricing = async () => {
    ...
    if (routeIds.length === 0) { setSeasonalPricing([]); return; }
    const { data } = await supabase.from('seasonal_pricing').select('*').in('route_id', routeIds).order('created_at', { ascending: false });
    setSeasonalPricing(data || []);
  };

  const fetchPriceHistory = async (routeId: string) => {
    const { data } = await supabase.from("route_price_history").select("*").eq("route_id", routeId).order("created_at", { ascending: false }).limit(20);
    ...
    if (editRoute) {
      ...
      if (oldPrice !== newPrice) {
        await supabase.from("route_price_history").insert({
          route_id: editRoute.id, operator_id: operator.id,
          old_price: oldPrice, new_price: newPrice, price_type: 'one_way', reason: 'Price update',
        });
      }
      ...
      if (newRoute) {
        await supabase.from("route_price_history").insert({
          route_id: newRoute.id, operator_id: operator.id,
          old_price: null, new_price: newPrice, price_type: 'one_way', reason: 'Route created',
        });
      }
  ...
  const toggleStatus = async (r: RouteRow) => {
    ...
    toast.success(`Route ${newStatus === "active" ? "activated" : "disabled"}`); fetchRoutes();
  };
  ...
  const handleSaveSeasonal = async () => {
    if (!operator || !selectedRouteForSeason || !seasonForm.season_name || !seasonForm.start_date || !seasonForm.end_date || !seasonForm.price_modifier) return;
    await supabase.from("seasonal_pricing").insert({
      route_id: selectedRouteForSeason, operator_id: operator.id,
      season_name: seasonForm.season_name, start_date: seasonForm.start_date, end_date: seasonForm.end_date,
      price_modifier: Number(seasonForm.price_modifier), is_percentage: seasonForm.is_percentage,
    });
    toast.success("Seasonal pricing added"); setSeasonalDialogOpen(false); fetchSeasonalPricing();
  };

  const toggleSeasonalActive = async (sp: SeasonalPricing) => {
    await supabase.from("seasonal_pricing").update({ is_active: !sp.is_active }).eq("id", sp.id); fetchSeasonalPricing();
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div><h1 className="text-2xl font-bold">Route Management</h1><p className="text-sm text-muted-foreground">Configure routes, pricing, and seasonal adjustments</p></div>
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab}>
                    ...
                    <TableRow key={r.id}>
                      ...
                      <TableCell className="text-right">
                        <div className="flex items-center justify-end gap-1">
                          ...
                          <Button variant="ghost" size="icon" onClick={() => openAddSeasonal(r.id)}><Calendar className="h-4 w-4" /></Button>
                          <Button variant="ghost" size="icon" onClick={() => { setSelectedRouteForHistory(r.id); setActiveTab('history'); }}><History className="h-4 w-4" /></Button>
        ...
        <TabsContent value="seasonal">
          <Card>
            ...
            <CardContent>
              {seasonalPricing.length === 0 ? <div className="text-center py-8 text-muted-foreground">No seasonal pricing</div> :
              <Table>
                ...
                <TableBody>
                  {seasonalPricing.map(sp => {
                    ...
                    return (
                    <TableRow key={sp.id}>
                      <TableCell>{route ? `${route.origin} → ${route.destination}` : 'Unknown'}</TableCell>
                      ...
                      <TableCell className="text-right"><Button variant="ghost" size="icon" onClick={() => toggleSeasonalActive(sp)}>{sp.is_active ? <Ban className="h-4 w-4" /> : <CheckCircle className="h-4 w-4" />}</Button></TableCell>
                    </TableRow>);
                  })}
                </TableBody>
              </Table>}
        ...
        <TabsContent value="history">
          <Card>
            ...
            <CardContent>
              {!selectedRouteForHistory ? <div className="text-center py-8 text-muted-foreground">Select a route</div> :
              priceHistory.length === 0 ? <div className="text-center py-8 text-muted-foreground">No history</div> :
              <Table>
                <TableHeader><TableRow><TableHead>Date</TableHead><TableHead>Type</TableHead><TableHead>Old</TableHead><TableHead>New</TableHead><TableHead>Reason</TableHead></TableRow></TableHeader>
                <TableBody>
                  {priceHistory.map(ph => (
                    <TableRow key={ph.id}>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent>
          ...
          <div className="space-y-4">
            <div><Label>Origin</Label><Select value={origin} onValueChange={setOrigin}><SelectTrigger><SelectValue placeholder="Select origin" /></SelectTrigger><SelectContent>{MALAWI_CITIES.filter(c => c !== destination).map(city => <SelectItem key={city} value={city}>{city}</SelectItem>)}</SelectContent></Select></div>
            <div><Label>Destination</Label><Select value={destination} onValueChange={setDestination}><SelectTrigger><SelectValue placeholder="Select destination" /></SelectTrigger><SelectContent>{MALAWI_CITIES.filter(c => c !== origin).map(city => <SelectItem key={city} value={city}>{city}</SelectItem>)}</SelectContent></Select></div>
            <div className="grid grid-cols-2 gap-3">
              <div><Label>One-Way (MWK)</Label><Input type="number" value={oneWay} onChange={e => setOneWay(e.target.value)} placeholder="15000" /></div>
              <div><Label>Return (MWK)</Label><Input type="number" value={returnPrice} onChange={e => setReturnPrice(e.target.value)} placeholder="25000" /></div>
            </div>
      ...
      <Dialog open={seasonalDialogOpen} onOpenChange={setSeasonalDialogOpen}>
        <DialogContent>
          ...
          <div className="space-y-4">
            <Select value={selectedRouteForSeason} onValueChange={setSelectedRouteForSeason}><SelectTrigger><SelectValue placeholder="Select route" /></SelectTrigger><SelectContent>{routes.map(r => <SelectItem key={r.id} value={r.id}>{r.origin} → {r.destination}</SelectItem>)}</SelectContent></Select>
            <Input placeholder="Season name" value={seasonForm.season_name} onChange={e => setSeasonForm({...seasonForm, season_name: e.target.value})} />
            <div className="grid grid-cols-2 gap-3">
              <div><Label>Start</Label><Input type="date" value={seasonForm.start_date} onChange={e => setSeasonForm({...seasonForm, start_date: e.target.value})} /></div>
              <div><Label>End</Label><Input type="date" value={seasonForm.end_date} onChange={e => setSeasonForm({...seasonForm, end_date: e.target.value})} /></div>
            </div>
            <Input type="number" placeholder="Modifier" value={seasonForm.price_modifier} onChange={e => setSeasonForm({...seasonForm, price_modifier: e.target.value})} />
            <Button onClick={handleSaveSeasonal} className="w-full">Add Season</Button>
          </div>
        </DialogContent>
      </Dialog>
    </div>
