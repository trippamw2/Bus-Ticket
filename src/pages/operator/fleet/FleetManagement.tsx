...
const FleetManagement = () => {
  ...
  useEffect(() => {
    if (operator) { fetchBuses(); fetchDrivers(); fetchDocuments(); fetchMaintenanceLogs(); }
  }, [operator, operator?.id]);

  const fetchBuses = async () => {
    if (!operator) return;
    const { data } = await supabase.from("buses").select("*").eq("operator_id", operator.id).order("created_at", { ascending: false });
    setBuses(data || []); setLoading(false);
  };

  const fetchDrivers = async () => {
    if (!operator) return;
    const { data } = await supabase.from("drivers").select("*").eq("operator_id", operator.id).order("created_at", { ascending: false });
    setDrivers(data || []);
  };
  ...
  const fetchDocuments = async () => {
    ...
    if (busIds.length === 0) { setDocuments([]); return; }
    const { data } = await supabase.from('bus_documents').select('*').in('bus_id', busIds).order('created_at', { ascending: false });
    setDocuments(data || []);
  };
  ...
  const fetchMaintenanceLogs = async () => {
    ...
    const { data: operatorBuses } = await supabase.from('buses').select('id').eq('operator_id', operator.id);
    const busIds = operatorBuses?.map(b => b.id) || [];
    if (busIds.length === 0) { setMaintenanceLogs([]); return; }
    const { data } = await supabase.from('maintenance_logs').select('*').in('bus_id', busIds).order('performed_date', { ascending: false });
    setMaintenanceLogs(data || []);
  };

  const openAddBus = () => { setEditBus(null); setPlate(""); setCapacity(""); setDialogOpen(true); };
  const openEditBus = (bus: BusItem) => { setEditBus(bus); setPlate(bus.plate_number); setCapacity(String(bus.capacity)); setDialogOpen(true); };

  const handleSaveBus = async () => {
    if (!operator || !plate.trim() || !capacity.trim()) return;
    setSaving(true);
    if (editBus) {
      await supabase.from("buses").update({ plate_number: plate.trim().toUpperCase(), capacity: Number(capacity) }).eq("id", editBus.id);
      toast.success("Bus updated");
    } else {
      await supabase.from("buses").insert({ plate_number: plate.trim().toUpperCase(), capacity: Number(capacity), operator_id: operator.id });
      toast.success("Bus added");
    }
    setSaving(false); setDialogOpen(false); fetchBuses();
  };

  const toggleBusStatus = async (bus: BusItem) => {
    const newStatus = bus.status === "active" ? "disabled" : "active";
    await supabase.from("buses").update({ status: newStatus }).eq("id", bus.id);
    toast.success(`Bus ${newStatus === "active" ? "activated" : "disabled"}`); fetchBuses();
  };

  const deleteBus = async (bus: BusItem) => {
    if (!confirm(`Delete bus ${bus.plate_number}?`)) return;
    await supabase.from("buses").delete().eq("id", bus.id);
    toast.success("Bus deleted"); fetchBuses();
  };
  ...
  const toggleDriverStatus = async (driver: Driver) => {
    ...
    await supabase.from("drivers").update({ status: newStatus }).eq("id", driver.id); fetchDrivers();
  };
  ...
  const handleSaveDoc = async () => {
    if (!operator || !selectedBusForDoc || !docForm.document_type) return;
    await supabase.from("bus_documents").insert({
      bus_id: selectedBusForDoc,
    ...
    toast.success("Document added"); setDocumentDialogOpen(false); fetchDocuments();
  };
  ...
  const handleSaveMaint = async () => {
    if (!operator || !selectedBusForMaint || !maintForm.maintenance_type) return;
    await supabase.from("maintenance_logs").insert({
      bus_id: selectedBusForMaint,
      operator_id: operator.id,
    ...
    toast.success("Maintenance log added"); setMaintenanceDialogOpen(false); fetchMaintenanceLogs();
  };

  const getStatusBadge = (status: string) => <Badge variant={status === 'active' ? 'default' : 'secondary'}>{status}</Badge>;
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div><h1 className="text-2xl font-bold">Fleet Management</h1><p className="text-sm text-muted-foreground">Manage buses, drivers, documents, and maintenance</p></div>
      </div>
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab}>
        ...
        <TabsContent value="drivers">
          <Card>
            ...
            <CardContent>
              {drivers.length === 0 ? <div className="text-center py-8 text-muted-foreground">No drivers</div> :
              <Table>
        ...
        <TabsContent value="documents">
          <Card>
            <CardHeader className="flex flex-row items-center justify-between">
              ...
              <Select onValueChange={(v) => openAddDoc(v)}><SelectTrigger className="w-[200px]"><SelectValue placeholder="Add document to..." /></SelectTrigger><SelectContent>{buses.map(b => <SelectItem key={b.id} value={b.id}>{b.plate_number}</SelectItem>)}</SelectContent></Select>
            </CardHeader>
            <CardContent>
        ...
        <TabsContent value="maintenance">
          <Card>
            <CardHeader className="flex flex-row items-center justify-between">
              ...
              <Select onValueChange={(v) => openAddMaint(v)}><SelectTrigger className="w-[200px]"><SelectValue placeholder="Log maintenance for..." /></SelectTrigger><SelectContent>{buses.map(b => <SelectItem key={b.id} value={b.id}>{b.plate_number}</SelectItem>)}</SelectContent></Select>
            </CardHeader>
            <CardContent>
              {maintenanceLogs.length === 0 ? <div className="text-center py-8 text-muted-foreground">No maintenance logs</div> :
              <Table>
                <TableHeader><TableRow><TableHead>Bus</TableHead><TableHead>Type</TableHead><TableHead>Cost</TableHead><TableHead>Date</TableHead><TableHead>Next Due</TableHead><TableHead>Status</TableHead></TableRow></TableHeader>
                <TableBody>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent>
          <DialogHeader><DialogTitle>{editBus ? "Edit Bus" : "Add Bus"}</DialogTitle></DialogHeader>
          <div className="space-y-4">
            <div><Label>Plate Number</Label><Input value={plate} onChange={e => setPlate(e.target.value)} placeholder="MZ 1234" /></div>
            <div><Label>Capacity</Label><Input type="number" value={capacity} onChange={e => setCapacity(e.target.value)} placeholder="50" /></div>
            <Button onClick={handleSaveBus} className="w-full" disabled={saving || !plate || !capacity}>{saving && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}{editBus ? "Update" : "Add"}</Button>
          </div>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={driverDialogOpen} onOpenChange={setDriverDialogOpen}>
        <DialogContent>
          ...
          <div className="space-y-4">
            <div><Label>Full Name</Label><Input value={driverForm.full_name} onChange={e => setDriverForm({...driverForm, full_name: e.target.value})} /></div>
            <div><Label>Phone</Label><Input value={driverForm.phone} onChange={e => setDriverForm({...driverForm, phone: e.target.value})} placeholder="+265..." /></div>
            <div><Label>License Number</Label><Input value={driverForm.license_number} onChange={e => setDriverForm({...driverForm, license_number: e.target.value})} /></div>
            <div><Label>License Expiry</Label><Input type="date" value={driverForm.license_expiry} onChange={e => setDriverForm({...driverForm, license_expiry: e.target.value})} /></div>
            <Button onClick={handleSaveDriver} className="w-full" disabled={!driverForm.full_name || !driverForm.phone}>{editDriver ? "Update" : "Add"} Driver</Button>
          </div>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={documentDialogOpen} onOpenChange={setDocumentDialogOpen}>
        <DialogContent>
          ...
          <div className="space-y-4">
            <Select value={selectedBusForDoc} onValueChange={setSelectedBusForDoc}><SelectTrigger><SelectValue placeholder="Select bus" /></SelectTrigger><SelectContent>{buses.map(b => <SelectItem key={b.id} value={b.id}>{b.plate_number}</SelectItem>)}</SelectContent></Select>
            <Select value={docForm.document_type} onValueChange={v => setDocForm({...docForm, document_type: v})}><SelectTrigger><SelectValue placeholder="Document type" /></SelectTrigger><SelectContent>{DOCUMENT_TYPES.map(t => <SelectItem key={t.value} value={t.value}>{t.label}</SelectItem>)}</SelectContent></Select>
      ...
      <Dialog open={maintenanceDialogOpen} onOpenChange={setMaintenanceDialogOpen}>
        <DialogContent>
          ...
          <div className="space-y-4">
            <Select value={selectedBusForMaint} onValueChange={setSelectedBusForMaint}><SelectTrigger><SelectValue placeholder="Select bus" /></SelectTrigger><SelectContent>{buses.map(b => <SelectItem key={b.id} value={b.id}>{b.plate_number}</SelectItem>)}</SelectContent></Select>
            <Select value={maintForm.maintenance_type} onValueChange={v => setMaintForm({...maintForm, maintenance_type: v})}><SelectTrigger><SelectValue placeholder="Type" /></SelectTrigger><SelectContent>{MAINTENANCE_TYPES.map(t => <SelectItem key={t.value} value={t.value}>{t.label}</SelectItem>)}</SelectContent></Select>
